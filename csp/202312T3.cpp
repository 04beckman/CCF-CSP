...
int p[2010];                                 // 存储父节点的数组
vector<long long> sum(2010);                 // 存储每个节点子树权重之和的向量
...
// 更新移除节点后的权重和祖先集合
void ddfs(int x, long long temp, vector<long long> &tsum, unordered_map<int, unordered_set<int>> &tallc, int target)
{
    if (x != target)
    {
        tsum[x] -= temp;
